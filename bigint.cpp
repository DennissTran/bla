...
struct bigint {
    ...
    string tostr() const{
        string s="";
        for(int i=0;i<a.size();i++)s=tostring(a[i])+s;
        return s;
    }
...
void solve()
{
    ...
    for(int i=0;i<s.size();i++)
    {
        ...
        {
            //cout<<so<<'\n';
            if(st.size()>1&&st.top()=="*"&&so!="")
            {
                    st.pop();
                    bigint now=so,se=st.top();
                    st.pop();
                    now*=se;
                    //cout<<now<<' '<<now.tostr()<<'\n';
                    st.push(now.tostr());
            ...
            if(s[i]==')')
            {
                bigint now=st.top();st.pop();
                while(1)
                {
                    if(st.top()=="("){st.pop();break;}
                    string kk=st.top();st.pop();
                    bigint se=st.top();st.pop();
                    if(kk=="*")now*=se;
                    else now+=se;
                    if(st.top()=="("){st.pop();break;}
                    //cout<<now<<' '<<now.tostr()<<'\n';
